...
idx_start = content.find("if (_showMixerControls) ...[")
if idx_start != -1:
    ...
    if idx_end != -1:
        ...
        # Let's just find the `Align` block for the mixer
        align_idx_start = content.find("Align(\n              alignment: Alignment.bottomCenter,", idx_start)
        align_idx_end = content.find("              ),\n            ),\n          ],", align_idx_start)
        if align_idx_end != -1:
            align_idx_end += len("              ),\n            ),\n          ],")
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    ElevatedButton(
                      ...
                      child: const Text('Apply Mixing', style: TextStyle(fontSize: 11, fontWeight: FontWeight.bold)),
                    ),
                  ],
                ),
              ),
            ...
            content = content[:align_idx_start] + new_mixer_ui + content[align_idx_end:]
            with open('client_flutter/lib/features/record/record_screen.dart', 'w') as f:
                f.write(content)
            print("Successfully updated Mixer Layout")
        else:
            print("align_idx_end not found")
            print(content[align_idx_start:align_idx_start+2000])
