...
class NativeKaraokeAudioEngine implements KaraokeAudioEngine {
 static const MethodChannel _channel =
  ...
  //
  // Flutter's EventChannel.receiveBroadcastStream() registers its message
  // handler keyed by CHANNEL NAME on the shared BinaryMessenger — that
  // registration is global, not per-Stream/per-instance. If engine
  // instance A is still listening and a DIFFERENT (older, disposing)
  // engine instance B's subscription to the same channel name gets
  // cancel()-ed afterwards (dispose() is async and can land late — e.g.
  // right after "Merekam Ulang" pushes a brand new RecordScreen while the
  // old EditRecordingScreen is still tearing down), B's cancel() call
  // wipes the shared message handler for that channel name — silently
  // killing event delivery for A too, even though A never asked to stop
  // listening. This is what caused the seekbar to freeze on re-record.
  //
  // Fix: create the real platform-level listen() exactly ONCE for the
  // whole app lifetime (mirrors the native engine's own singleton
  // design), and fan events out through our own plain Dart broadcast
  // controllers. Every screen/instance only ever subscribes to/cancels
  // those local controllers, which never touch the platform channel.
  static StreamSubscription? _rawPositionSub;
  static StreamSubscription? _rawPitchSub;
  static final StreamController<double> _rawPositionStream =
      StreamController<double>.broadcast();
  static final StreamController<Map<dynamic, dynamic>> _rawPitchStream =
      StreamController<Map<dynamic, dynamic>>.broadcast();

  static void _ensureRawSubscriptions() {
    _rawPositionSub ??= _positionChannel.receiveBroadcastStream().listen(
      (dynamic event) {
        if (event is num) {
          _rawPositionStream.add(event.toDouble());
        }
      },
      onError: (Object error, StackTrace stack) {
        print('❌ RAW POSITION EVENT ERROR: $error');
      },
      cancelOnError: false,
    );

    _rawPitchSub ??= _pitchChannel.receiveBroadcastStream().listen(
      (dynamic event) {
        if (event is Map) {
          _rawPitchStream.add(event);
        }
      },
      onError: (Object error, StackTrace stack) {
        print('❌ RAW PITCH EVENT ERROR: $error');
      },
      cancelOnError: false,
    );
  }
...
Future<void> _createPositionSubscription() async {
  print('🔥 Creating POSITION subscription (shared raw stream)...');

  _ensureRawSubscriptions();
  ...
  _positionSub = _rawPositionStream.stream.listen(
    (double seconds) {
      if (_disposed) {
        return;
      }
  ...
  print(
    '🔥 POSITION subscription CREATED '
    'sub=${_positionSub != null}',
  );
}
  Future<void> _createPitchSubscription() async {
  print('🔥 Creating PITCH subscription (shared raw stream)...');
  ...
  _pitchSub = _rawPitchStream.stream.listen(
  ...
  print('🔥 PITCH subscription CREATED');
}
// await _channel.invokeMethod('init');
...
Future<void> startRecording() async {
  ...
  void setEQBand(int bandIndex, double frequency, double gain, double q) {
    _channel.invokeMethod('setEQBand', {
      'bandIndex': bandIndex,
      'frequency': frequency,
      'gain': gain,
      'q': q,
